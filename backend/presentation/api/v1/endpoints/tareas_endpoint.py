# ═══════════════════════════════════════════════════════════════════════════════
# CAPA: PRESENTATION — Endpoints FastAPI (HTTP /api/v1/tareas)
#
...
#
# ¿Por qué presentation/ y no adapters/?
#   La capa de Presentation es la que "habla HTTP". Los endpoints FastAPI son
#   el punto de entrada de la API REST — son Presentation, no Adapters.
#   Adapters sería si adaptáramos un protocolo externo (WebSocket, gRPC, CLI).
#   En Clean Architecture aplicada a APIs REST, los endpoints HTTP van en Presentation.
#
# ¿Por qué /api/v1/?
#   El prefijo "v1" versiona la API. Si mañana cambia el contrato (campos nuevos,
#   campos eliminados), podemos crear /api/v2/ sin romper los clientes que usan /api/v1/.
#   Es una práctica estándar en APIs REST profesionales.
#
...
# El endpoint crea la instancia con la sesión de DB inyectada.
...
from presentation.schemas.tareas_schemas import TareaCreateRequest, TareaUpdateRequest, TareaResponse
# Los DTOs Pydantic (viven en presentation/schemas/):
...
router = APIRouter(
    ...
    # Todos los endpoints de este router comenzarán con "/api/v1/tareas".
    # El "v1" versiona la API: si el contrato cambia en el futuro,
    # se crea /api/v2/tareas sin romper clientes que usan /api/v1/tareas.
...
# @router.get("/") → responde a GET /api/v1/tareas/
...
def listar_tareas(db: Session = Depends(get_db)):
    """
    GET /api/v1/tareas/ — Devuelve todas las Tareas.
...
# POST /api/v1/tareas/ — crea un recurso nuevo → HTTP 201 Created.
def crear_tarea(body: TareaCreateRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        #
        # Nota: en sistemas más complejos, usaríamos excepciones de dominio
        # como clases propias (ej: TituloInvalidoError) que almacenan datos
        # estructurados como atributos (self.titulo, self.min_length).
        # Aquí usamos ValueError por simplicidad pedagógica.
        raise HTTPException(
...
# {id} en el path = parámetro de ruta. GET /api/v1/tareas/f47ac10b-... → id="f47ac10b-..."
def obtener_tarea(id: str, db: Session = Depends(get_db)):
    """
    GET /api/v1/tareas/{id} — Obtiene una Tarea específica por ID.
    """
    repo = TareaRepositoryImpl(db)
...
# PUT /api/v1/tareas/{id} — actualiza un recurso completo (reemplaza los campos enviados).
def actualizar_tarea(id: str, body: TareaUpdateRequest, db: Session = Depends(get_db)):
    """
    PUT /api/v1/tareas/{id} — Actualiza título y/o estado de una Tarea.

    id: str   → viene del path (/api/v1/tareas/f47ac10b-...)
...
# DELETE /api/v1/tareas/{id} — elimina un recurso.
...
def eliminar_tarea(id: str, db: Session = Depends(get_db)):
    """
    DELETE /api/v1/tareas/{id} — Elimina una Tarea por ID.
    """
    repo = TareaRepositoryImpl(db)
