...
//
// ✅ Puede importar: domain/entities, application/dto, application/ports
...
import type { CrearTareaInput } from '@/application/dto/inputTareaDto'
...
export class CrearTarea {
  ...
  async execute(input: CrearTareaInput): Promise<TareaOutput> {
    ...
    // ": Promise<TareaOutput>" → TypeScript garantiza que devolvemos una Promise de TareaOutput.
    //   TareaOutput = el OUTPUT DTO: datos puros que viajan hacia afuera (hacia el hook y la UI).
    //   TareaOutput ≠ TareaEntity: la Entity tiene métodos y lógica; el Output DTO es solo datos.
    //   "<TareaOutput>" es un generic — TypeScript puro, desaparece al compilar.
    ...
    // Con "await": esperamos el resultado real (la TareaOutput creada).
    //
    ...
    // Solo sabe que le prometieron un "crear()" que devuelve Promise<TareaOutput>.
  }
}
